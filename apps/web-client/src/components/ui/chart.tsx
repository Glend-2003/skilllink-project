...
// Define tipos para los payloads
...
function ChartTooltipContent({
  ...
  const tooltipLabel = React.useMemo(() => {
    if (hideLabel || !tooltipPayload || !tooltipPayload.length) {
      return null;
    }

    const [item] = tooltipPayload as TooltipPayloadItem[];
    ...
    if (labelFormatter) {
      return (
        <div className={cn("font-medium", labelClassName)}>
          {labelFormatter(value as any, tooltipPayload)}
        </div>
      );
    }
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-1.5">
        {(tooltipPayload as TooltipPayloadItem[]).map((item: TooltipPayloadItem, index: number) => {
...
// Define tipos para legend payload
